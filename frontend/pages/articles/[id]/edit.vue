  ...
  <div class="article-edit-page">
    ...
    <div v-else>
      <div class="page-header">
        <h1 class="page-title">記事を編集</h1>
        <NuxtLink :to="`/articles/${articleId}`" class="back-link">
          ← 記事詳細に戻る
        </NuxtLink>
      </div>
        ...
        <div class="form-layout">
          ...
          <div class="form-section">
            ...
            <div class="form-actions">
              ...
              <NuxtLink :to="`/articles/${articleId}`" class="btn btn-cancel">
                キャンセル
              </NuxtLink>
            </div>
...
const fetchArticle = async () => {
  ...
  try {
    ...
    loadError.value = err instanceof Error ? err.message : '記事の取得に失敗しました'
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.message || '記事の更新に失敗しました')
    }

    // 更新した記事の詳細ページへリダイレクト
    router.push(`/articles/${articleId}`)
  } catch (error) {
    submitError.value = error instanceof Error ? error.message : '記事の更新に失敗しました'
...
// SEO対策
useHead({
  title: computed(() => formData.value.title ? `${formData.value.title}を編集 - ブログアプリ` : '記事を編集 - ブログアプリ'),
  meta: computed(() => [
    {
      name: 'description',
      content: '記事を編集します'
